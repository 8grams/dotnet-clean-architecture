...
namespace SFIDWebAPI.Application.UseCases.User.Guide.Category.Models
{
    public class GuideCategoryData : BaseDtoData, IHaveCustomMapping
    {
        ...
        {
            configuration.CreateMap<Domain.Entities.MasterCar, GuideCategoryData>()
                .ForMember(cDto => cDto.Id, opt => opt.MapFrom(c => c.Id))
                .ForMember(cDto => cDto.Title, opt => opt.MapFrom(c => c.Title))
                .ForMember(cDto => cDto.Tag, opt => opt.MapFrom(c => c.Tag))
                .ForMember(cDto => cDto.TotalMaterials, opt => opt.MapFrom(c => c.GuideMaterials.Count))
                ...
                {
                    Reading = c.GuideMaterials.Where(e => e.FileType != "mp4").Count(),
                    Video = c.GuideMaterials.Where(e => e.FileType == "mp4").Count(),
                }))
