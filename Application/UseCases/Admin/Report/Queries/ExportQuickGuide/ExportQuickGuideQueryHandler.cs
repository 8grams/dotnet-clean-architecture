...
{
    public class ExportQuickGuideQueryHandler : IRequestHandler<ExportQuickGuideQuery, ExportQuickGuideDto>
    {
        ...
        public ExportQuickGuideQueryHandler(ISFDDBContext context, IMediator mediator, IUploader uploader, IWebHostEnvironment env, Utils utils)
        {
            _context = context;
            _mediator = mediator;
            _uploader = uploader;
        ...
        public async Task<ExportQuickGuideDto> Handle(ExportQuickGuideQuery request, CancellationToken cancellationToken)
        {
            var dataQueryQ =  _context.MaterialCounters
                .Where(e => e.CreateDate >= request.StartDate)
                .Where(e => e.CreateDate <= request.EndDate);

            var userGuides = _context.MaterialCounters
            ...
            var users = _context.Users
                .Where(e => userGuides.Contains(e.Id))
                .Select(e => e.UserName)
                .ToList();
            ...
            var path = Utils.GetUploadLocation("export", "export_guide.csv");
            ...
            var a = await Task.FromResult<ExportQuickGuideDto>(null);
            return new ExportQuickGuideDto
            {
                Success = true,
                Message = "Export Bulletin has been successfully executed",
                Data = new ExportQuickGuideData
                {
